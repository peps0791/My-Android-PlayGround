...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public Map<Integer, List<List<Float>>> fetchDownloadDataList(int timestamp, int limit){

        Log.d(Constants.CUSTOM_LOG_TYPE, "fetch download data function called with table name ::" +tableName);
        Cursor cur = db.rawQuery("SELECT * FROM " + tableName + " ORDER BY TIMESTAMP DESC LIMIT " +
                String.valueOf(limit), null);
        List<Float> xList = new ArrayList();
        List<Float> yList = new ArrayList();
        List<Float> zList = new ArrayList();
